...
static void expidus_shell_desktop_method_handler(FlMethodChannel* channel, FlMethodCall* call, gpointer data) {
  ...
  if (!g_strcmp0(fl_method_call_get_name(call), "toggleActionButton")) {
    FlValue* val = fl_method_call_get_args(call);
    if (fl_value_get_type(val) != FL_VALUE_TYPE_BOOL) {
      if (!fl_method_call_respond(call, FL_METHOD_RESPONSE(fl_method_error_response_new("args.invalid", "Value must be a boolean", val)), &error)) {
        g_error("Failed to respond to call: %s", error->message);
        g_clear_error(&error);
      }
      return;
    }
    gboolean is_alt = fl_value_get_bool(val);
    if (is_alt) {
      ExpidusShell* shell;
